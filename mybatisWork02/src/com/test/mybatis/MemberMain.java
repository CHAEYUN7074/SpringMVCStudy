...
public class MemberMain
{
	...
	@RequestMapping(value = "/memberlist.action", method = RequestMethod.GET) //---액션을 요청할때
	public String memberList(ModelMap model)
	{
		
		IMemberDAO dao = sqlSession.getMapper(IMemberDAO.class);
		//IMemberDAO dao = (IMemberDAO)new MemberDAO(); -업캐스팅
		...
		model.addAttribute("count", dao.count()); //"이름"
	...
	{
		IMemberDAO dao = sqlSession.getMapper(IMemberDAO.class); //DAO생성
		dao.remove(m);
	...
	public String memberUpdate(MemberDTO m)
	{
		...
		return "redirect:memberlist.action"; //:부분 항상 붙여써야한다!
	} //이제 DAO XML 가서 선언만 한 메소드의 쿼리문을 만들어주자
}
